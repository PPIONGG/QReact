...
import { createVisitReport, getListVisited, updateVisitReport, VisitDetailsAPI } from '../api/api';
import {
  ResListVisited,
  ResUpdateVisit,
  ResVisitDetail,
...
export interface SalesVisitorStore {
  ...
  saveVisitReport: (requestBody: UpdateVisitRequest, mode: 'create' | 'update') => Promise<ResUpdateVisit>;
  clearSaveError: () => void;
...
export const useSalesVisitorStore = create<SalesVisitorStore>((set) => ({
  ...
  saveVisitReport: async (requestBody: UpdateVisitRequest, mode: 'create' | 'update') => {
    set({ saveLoading: true, saveError: null, saveResponse: null });

    try {
      // เลือก API function ตาม mode
      const apiFunction = mode === 'create' ? createVisitReport : updateVisitReport;
