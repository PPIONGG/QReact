...
const LogVisited: React.FC = () => {
  ...
  useEffect(() => {
    fetchListVisited('S001');
  }, [fetchListVisited]);

  //   useEffect(() => {
  //   // ดึง salesCode จาก session storage
  ...
  //         const salesCode = parsedData?.state?.Salesinfo?.data?.salesCode || null;
  //         console.log('Parsed salesCode:', salesCode);
  //         return salesCode;
  //       }
  //       return null;
  //     } catch (error) {
  ...
  //   const salesCode = getSessionData();
  //   console.log('Sales Code from session:', salesCode);
    
  //   if (salesCode) {
  //     fetchListVisited(salesCode);
  //   } else {
  //     console.warn('No salesCode found in session storage');
  //   }
  // }, [fetchListVisited]);
      ...
      render: (_: any, record: TableDataItem) => (
        <Space>
          <Button
            ...
            icon={<EditOutlined />}
            size="small"
            onClick={() => handleEditReport(record.noItem)}
          >
            Edit
          </Button>
        </Space>
